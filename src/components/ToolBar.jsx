 ...
 *   onClick - 点击回调函数
 */
const ToolBarButton = ({ icon, alt, title, onClick, disabled }) => {
	// 工具栏按钮组件，接收icon、alt、title、onClick、disabled五个参数
	return (
		// 返回按钮JSX结构
		<button
...
const ToolBar = () => {
	...
	const handleUndo = () => {
		// 处理撤销点击（开发目标.txt第36行：撤销，第75行）
		window.cmd.undo(); // 调用全局命令：撤销
	};

	const handleRedo = () => {
		// 处理反撤销点击（开发目标.txt第37行：反撤销，第76行）
		window.cmd.redo(); // 调用全局命令：重做
	};

	const handleZoomIn = () => {
		// 处理放大点击（开发目标.txt第38行：放大，第77行）
		window.cmd.zoomIn(); // 调用全局命令：放大
	};

	const handleZoomOut = () => {
		// 处理缩小点击（开发目标.txt第41行：缩小，第79行）
		window.cmd.zoomOut(); // 调用全局命令：缩小
	};

	const handleResetZoom = () => {
		// 处理重置缩放点击（开发目标.txt第39-40行：缩放数值显示，点击重置缩放，第78行）
		window.cmd.resetZoom(); // 调用全局命令：重置缩放
	};

	const handleArrange = () => {
		// 处理整理点击（开发目标.txt第42行：整理，第80行）
