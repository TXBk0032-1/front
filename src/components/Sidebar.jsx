...
function CategoryItem({ category, label, color }) {
	...
	return (
		...
		<div /* 分类项容器 */
			...
			style={{
				/* 内联样式 */ "--category-color": color /* CSS变量：分类颜色 */,
...
function CategoryBar() {
	...
	return (
		...
		<div className="category-bar">
			{" "}
			{/* 分类栏容器 */}
			<CategoryItem /* 全部分类项 */ category="all" /* 分类键名 */ label="全部" /* 显示名称 */ color="#8b92e5" /* 颜色 */ />
			{categories.map((cat /* 遍历所有分类 */) => (
				<CategoryItem /* 分类项 */ key={cat.key} /* React key */ category={cat.key} /* 分类键名 */ label={cat.label} /* 显示名称 */ color={cat.color} /* 颜色 */ />
			))}
		</div>
	);
}
...
function NodeGroup({ label, color, nodes }) {
	...
	return (
		...
		<div className="node-group">
			{" "}
			{/* 节点分组容器 */}
			<div /* 分组标题 */ className="group-title" /* 样式类名 */ style={{ "--group-color": color }} /* 内联样式，传入颜色变量 */>
				{label} {/* 显示分组名称 */}
			</div>
			{nodes.map((node /* 遍历该分组的所有节点 */) => (
				<NodeItem /* 节点项 */ key={node.opcode} /* React key */ node={node} /* 节点数据 */ color={color} /* 节点颜色 */ />
			))}
		</div>
	);
}
...
function getGroupedNodes(registry, selectedCategory) {
	...
	categoryMap.forEach((catData, catId) => {
		// 遍历所有分类
