...
function Blueprint() {
   ...
   * @param {Event} e - 拖放事件
   */
  const handleDrop = (e) => {                                       // 处理拖放事件
    e.preventDefault()                                             // 阻止默认行为

    const opcode = e.dataTransfer.getData('opcode')                // 获取节点opcode
    if (!opcode) return                                            // 如果没有opcode，直接返回

    const position = screenToFlowPosition({                        // 将屏幕坐标转为画布坐标
      x: e.clientX,                                                // 鼠标X坐标
      y: e.clientY                                                 // 鼠标Y坐标
    })

    createNode(opcode, {                                           // 创建节点
      x: position.x,                                               // 节点X坐标
      y: position.y                                                // 节点Y坐标
    })
  }
