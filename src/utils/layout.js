...
function assignLayers(nodeMap, inEdges, outEdges) {
  ...
  if (isolatedNodes.length > 0) {
    ...
    levels[0] = [...isolatedNodes, ...levels[0]];
  }
...
function edgesCross(edge1, edge2, nodeMap) {
  ...
  // 交叉条件：起点顺序与终点顺序相反
  return (
    (y1_from < y2_from && y1_to > y2_to) || (y1_from > y2_from && y1_to < y2_to)
  );
}
  ...
  if (direction === "down") {
    ...
    for (const edge of incoming) {
      if (prevLayer.includes(edge.from)) {
        ...
        const targetY =
          fromNode.y + (fromPortY - fromNode.y) - (toPortY - node.y);
        sum += targetY;
        count++;
      }
    }
 ...
 * @param {Map} inEdges - 入边映射表
 * @param {Map} outEdges - 出边映射表
 * @returns {boolean} - 是否为孤立节点
 */
function isIsolatedNode(nodeId, inEdges, outEdges) {
  const incoming = inEdges.get(nodeId) || [];
  const outgoing = outEdges.get(nodeId) || [];
...
function calculateYCoordinates(levels, nodeMap, inEdges, outEdges, baseGapY) {
  ...
  for (let k = 0; k < levels.length; k++) {
    ...
    for (const nodeId of layer) {
      ...
      } else if (isIsolatedNode(nodeId, inEdges, outEdges)) {
        isolatedNodes.push(nodeId);
      } else {
        connectedNodes.push(nodeId);
      }
    ...
    for (let iter = 0; iter < 2; iter++) {
      for (let i = 0; i < connectedNodes.length; i++) {
        ...
        if (k > 0) {
          ...
          for (const edge of incoming) {
            if (levels[k - 1].includes(edge.from)) {
              const fromNode = nodeMap.get(edge.from);
              const fromPortY = getPortActualY(
                fromNode,
                edge.sourceHandle,
                false,
              );
        ...
        if (i > 0) {
          const prevNode = nodeMap.get(connectedNodes[i - 1]);
          minY = prevNode.y + prevNode.height + baseGapY;
        }

        if (i < connectedNodes.length - 1) {
          const nextNode = nodeMap.get(connectedNodes[i + 1]);
    ...
    // 方法C：将孤立节点放置在有连线节点的下方，紧邻它们
    if (isolatedNodes.length > 0) {
      // 计算有连线节点的Y范围
      let connectedMaxY = 0;
      if (connectedNodes.length > 0) {
        const lastConnectedNode = nodeMap.get(
          connectedNodes[connectedNodes.length - 1],
        );
        connectedMaxY =
          lastConnectedNode.y + lastConnectedNode.height + baseGapY;
      }

      // 将孤立节点紧凑地放置在有连线节点下方
      let isolatedY = connectedMaxY;
      for (const nodeId of isolatedNodes) {
        const node = nodeMap.get(nodeId);
        node.y = isolatedY;
        isolatedY += node.height + baseGapY;
      }
    }
...
export function arrangeNodes(nodes, edges, options = {}) {
  ...
  const layerY = calculateYCoordinates(
    levels,
    nodeMapWithDummies,
    inEdges,
    outEdges,
