...
- 代码要让人看得懂，别为了炫技写得很绕
- AI 可以帮忙补全代码，但别让 AI 一次性生成大段代码
- 注释要写大白话，别写得跟论文一样

## 二、项目目标

做一个 **AI 模型架构可视化设计工具**，就像 Scratch 那样拖拖拽拽就能搭建模型。
...
1. `NodeBox.jsx`：给节点加 `draggable`，拖拽时把节点 ID 存到 `dataTransfer`
2. `App.jsx`：监听 `onDrop`，获取节点 ID，把屏幕坐标转成画布坐标，创建节点
...
- `nodeRegistry.js`：加了 4 个辅助函数（`getNodeConfig`、`getNodeColor` 等），其他文件不用再写一堆查找逻辑
...
先尝试写一下导出和导入蓝图功能，先看看蓝图的格式是什么样的，因为要交给后端去识别格式。

---

### Step 15：添加顶部菜单栏和导入导出功能
...
修改了 `flowConfig.js`，新增两个样式配置：

- `APP_CONTAINER_STYLE`：最外层垂直布局
- `WORKSPACE_STYLE`：工作区水平布局
...
1. FlowCanvas 使用 `forwardRef` + `useImperativeHandle` 暴露方法
2. App 组件通过 ref 调用这些方法
3. 导出：获取 nodes/edges 数据，生成 JSON 下载
4. 导入：读取 JSON 文件，设置 nodes/edges 数据
...
```json
{
  "nodes": [...],           // 节点数组
  "edges": [...],           // 连线数组
  "nodeIdCounter": 100      // 下一个节点ID（避免ID冲突）
}
```
